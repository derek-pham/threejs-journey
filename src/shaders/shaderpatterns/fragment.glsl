...
void main() {
    ...
    // strength *= step(0.4, mod(10.0 * vUv.y, 1.0));
    ...
    // float strength = step(0.49, max(abs(vUv.y - 0.5), abs(vUv.x - 0.5)));
    ...
    // Pattern 25
    // vec2 gridUv = vec2(floor(vUv.x * 10.0) / 10.0, floor((vUv.y + vUv.x * 0.1) * 10.0) / 10.0);
    // float strength = random(gridUv);
    ...
    // Pattern 49
    // float strength = sin(cnoise(vUv * 30.0) * 20.0);

    // Pattern 50
    // float strength = step(0.8, sin(cnoise(vUv * 20.0) * 10.0));
